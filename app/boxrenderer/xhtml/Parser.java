...
public class Parser {
    ...
    private List<String> inherited = Lists.newArrayList();
    {
        inherited.add("font-family");
        inherited.add("font-size");
        inherited.add("font-weight");
        inherited.add("color");
    }
    ...
    private CssStyleFactory cssStyleFactory;

    public Parser() {
        // TODO use a default resolver
        fontManager = new FontManager(null);
        cssStyleFactory = new CssStyleFactory(null);
    }
    ...
    private Box setupBox(Box box, Box parent, Element el) throws Exception {
        ...
        if(!StringUtils.isBlank(border)) {
            style+=String.format("border-width:%spx;border-color:red;border-collapse:separate;", border);
        }
        setupCss(box, style);
        applyCss(box);
        return box;
    ...
    private Set<PropertyValue> getCss(Box box) {
        Map<String, PropertyValue> cssMap = getCss(box, new HashMap<String, PropertyValue>());
        Set<PropertyValue> result = new HashSet<PropertyValue>();
        result.addAll(cssMap.values());
        return result;
    }

    private Map<String, PropertyValue> getCss(Box box, Map<String, PropertyValue> result) {
        if(box == null) {
            return result;
        }
    ...
    private void addProperties(List<PropertyValue> properties, Map<String, PropertyValue> result) {
...
//      }
//      for(PropertyValue value : rule.getPropertyValues()) {
//          System.out.println(value.getProperty()+": "+value.getValue());
//      }
//      System.out.println("-------------------------");
//  }
        return rules;
